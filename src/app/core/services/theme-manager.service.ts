...
export class ThemeManagerService {
  ...
  getStoredTheme = () =>
    JSON.parse(this.browserStorage.getItem(LOCAL_STORAGE_KEY) ?? '{}').theme;
  setStoredTheme = (theme: string) => {
    const meta = JSON.parse(
      this.browserStorage.getItem(LOCAL_STORAGE_KEY) ?? '{}'
    );
    meta.theme = theme;
    this.browserStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(meta));
  };
